...
shorts_renderer.py — Renders a YouTube Short (9:16 vertical) using:
  ...
  - Overlays pre-baked into background frames via Pillow compositing
  - FFmpeg concat demuxer for final H.264 encoding (no overlay filter)
...
def _make_background_image(pillar: str = "investing"):
    """Generate gradient background with watermark. Returns RGBA PIL Image."""
    from PIL import Image, ImageDraw
    palette_map = {
...
def render(
    ...
    - Pre-bakes overlays into background frames using Pillow (fast, no FFmpeg filter graph)
    - Uses FFmpeg concat demuxer on static PNG segments for final encode
    ...
    # Filter out overlays that start after voiceover ends
    overlays = [ov for ov in script_data.get("overlays", []) if _ov_start(ov) < vo_duration]

    # Compute unique time boundaries from all overlay start/end points
    ...
    # For each segment: composite bg + active overlays → PNG
    from PIL import Image
    ...
    # FFmpeg concat demuxer requires the last entry to be repeated (no duration on final line)
    if concat_lines:
    ...
    # Build FFmpeg command: concat image segments + audio
    ...
    logger.info("Encoding Short video (%d segments, %d overlays)...",
                len(events_sorted) - 1, len(overlays))
